...
async def render_playoff_bracket(
    ...
    round_font = load_font("Bold", 18)
    team_font = load_font("Regular", 18)
    score_font = load_font("Black", 20)
    seed_font = load_font("Regular", 14)
    ...
    top_y = PADDING + 80
    # Vertical center positions for each series in each round -- round 1
    # series are evenly spaced; later rounds center between the two
    # series that feed into them, so connecting lines look right.
    ...
    for round_index, series_list in enumerate(rounds):
        ...
        draw.text((x, top_y - 30), series_list[0].round_name.upper(), font=round_font, fill=Theme.TEXT_MUTED)

        for i, series in enumerate(series_list):
            center_y = centers[round_index][i]
            box_y = center_y - SERIES_HEIGHT / 2
            box_w = ROUND_WIDTH - 30
            ...
            await _draw_team_row(img, draw, x, box_y + 8, box_w, team_a, series.seed_a, series.wins_a, a_won, team_font, seed_font, score_font)
            draw.line([(x + 8, box_y + SERIES_HEIGHT / 2), (x + box_w - 8, box_y + SERIES_HEIGHT / 2)], fill=Theme.BORDER, width=1)
            await _draw_team_row(
                img, draw, x, box_y + SERIES_HEIGHT / 2 + 8, box_w, team_b, series.seed_b, series.wins_b, b_won, team_font, seed_font, score_font
            )

            # Connector line to the next round, if there is one.
...
async def _draw_team_row(img, draw, x, y, box_w, team, seed, wins, won, team_font, seed_font, score_font) -> None:
    name_x = x + 10
    if team is not None:
    ...
    name = f"{seed_str}{team.name}" if team else "TBD"
    color = Theme.WIN_GREEN if won else Theme.TEXT_PRIMARY
    draw.text((name_x, y), name, font=team_font, fill=color)

    score_str = str(wins)
    score_w = draw.textlength(score_str, font=score_font)
    draw.text((x + box_w - 20 - score_w, y - 2), score_str, font=score_font, fill=color)
